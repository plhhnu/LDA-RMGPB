    ...
    for cv in range(3, 4):
        ...
        for j in range(time):
            ...
            for i in range(k):
                ...
                curve_1.to_csv('./result_cv3_gbdt/d2/d' + str(mm) + 'c' + str(cv) + '_c' + str(au) + '.csv',
                               header=None, index=None)

                curve_2 = np.vstack([rec_, pre])
                curve_2 = pd.DataFrame(curve_2.T)
                curve_2.to_csv('./result_cv3_gbdt/d2/d' + str(mm) + 'c' + str(cv) + '_r' + str(apr) + '.csv',
                               header=None, index=None)
        ...
        toa.to_csv('./result_cv3_gbdt/d2/' + 'cv' + str(cv) + '_data' + str(mm) + '.csv', header=None, index=None)
        PRE = mPREs.mean()
        ACC = mACCs.mean()
        REC = mRECs.mean()
        AUC = mAUCs.mean()
        ...
        f_1 = open('./result_cv3_gbdt/d2/' + 'res_cv.txt', 'a')
